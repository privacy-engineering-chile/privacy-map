...
import { Jurisdiction, REGION_COLORS, STATUS_COLOR, STATUS_LABEL } from "@/data/jurisdictions";
import {
  Table,
  TableBody,
  TableCell,
...
export const JurisdictionsTable = ({
  ...
  const exportCSV = () => {
    const header = "Jurisdiction,Region,SubRegion,Year,Status,KeyLaw,DPA\n";
    const body = sorted
      .map(
        (j) =>
          `"${j.jurisdiction}","${j.region}","${j.subRegion}",${j.year ?? ""},${j.lawStatus},"${(j.keyLawName ?? "").replace(/"/g, "'")}","${(j.dpa ?? "").replace(/"/g, "'")}"`,
      )
      .join("\n");
          ...
          <TableBody>
            {sorted.map((j) => (
              ...
              >
                ...
                <TableCell>
                  <Badge
                    style={{ background: STATUS_COLOR[j.lawStatus], color: "white" }}
                    className="text-[10px]"
                  >
